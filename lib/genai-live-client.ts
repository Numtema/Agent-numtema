...
// FIX: Refactor to use composition for EventEmitter to resolve issues with `on`, `off`, and `emit` methods.
export class GenAILiveClient {
  ...
  public async connect(config: LiveConnectConfig): Promise<boolean> {
    ...
    try {
      ...
      const errorEvent = new ErrorEvent('error', {
        error: e,
        message: e?.message || 'Échec de la connexion.',
      });
  ...
  public disconnect() {
    ...
    this.log('client.close', `Déconnecté`);
    return true;
  }

  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      this.emitter.emit('error', new ErrorEvent('Le client n\'est pas connecté'));
      return;
    }
  ...
  public sendRealtimeInput(chunks: Array<{ mimeType: string; data: string }>) {
    ...
    let message = 'inconnu';
    if (hasAudio && hasVideo) message = 'audio + vidéo';
    else if (hasAudio) message = 'audio';
    else if (hasVideo) message = 'vidéo';
  ...
  protected onError(e: ErrorEvent) {
    ...
    const message = `Impossible de se connecter à GenAI Live : ${e.message}`;
  ...
  protected onClose(e: CloseEvent) {
    ...
    this.log(
      `server.${e.type}`,
      `déconnecté ${reason ? `avec la raison : ${reason}` : ``}`
    );
